...
# 'data/gapminder.txt'
df = pd.read_csv(r'data/gapminder.txt', sep='\t')
print(df.shape)
df.info()
print(df.describe())
print('=' * 50)
...
# print((df.groupby('year').agg({'lifeExp': 'mean'}) == df.groupby('year').agg({'lifeExp': np.mean})).all())
print('=' * 50)
...
# todo 课下练习:
# 练习1: 求每年各个国家的平均年龄
print(df.groupby('country').agg({'lifeExp': np.mean}))
print('=' * 50)
# 练习2: 求每年各个国家的平均年龄和最高gdp
print(df.groupby('country').agg({'lifeExp': np.mean, 'gdpPercap': np.max}))
print('=' * 50)
# 练习3: 单独计算1952年的平均寿命
print(df[df['year'] == 1952].agg({'lifeExp': np.mean}))
